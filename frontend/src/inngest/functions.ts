    ...
    const { userId, credits, endpoint, body } = await step.run(
      ...
      async () => {
        ...
        if (song.fullDescribedSong) {
          endpoint = sanitizeUrl(env.GENERATE_FROM_DESCRIPTION);
          body = {
        ...
        // Custom mode: Lyrics + prompt
          ...
          endpoint = sanitizeUrl(env.GENERATE_WITH_LYRICS);
          body = {
            lyrics: song.lyrics,
            prompt: song.prompt,
            ...commomParams,
        ...
        // Custom mode: Prompt + described lyrics
          ...
          endpoint = sanitizeUrl(env.GENERATE_FROM_DESCRIBED_LYRICS);
          body = {
